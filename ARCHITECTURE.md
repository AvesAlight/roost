...
a PreCompact hook: on `trigger="auto"` it returns
...
and `custom_instructions` populated, which the hook passes through
— and the manual `/compact` actually steers the compactor with our
