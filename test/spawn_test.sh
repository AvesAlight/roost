...
# -- Test 19: --compact-directive echoed + file actually written -------------
# Per-agent compaction directive (issue #368). The diagnostic prints before
# the tmux/ircd preflight. ROOST_SPAWN_KEEP_DATA_DIR=1 is a test seam that
# echoes the data-dir path early and suppresses the failure-cleanup trap so
# we can read the file the spawn wrote and verify the spawn→file handoff.

setup
out="$(ROOST_SPAWN_KEEP_DATA_DIR=1 "${ROOST_BIN}" spawn testnick --cwd "$TDIR" --compact-directive 'retain X' 2>&1 || true)"
data_dir="$(echo "$out" | sed -n 's/.*data dir (preflight): //p' | head -1)"
if echo "$out" | grep -qF "compact directive: retain X" \
    && [ -n "$data_dir" ] && [ -f "$data_dir/compact-directive.txt" ] \
    && [ "$(cat "$data_dir/compact-directive.txt")" = "retain X" ]; then
  ok "--compact-directive: diagnostic + file written with expected body"
else
  fail "--compact-directive: diagnostic + file written with expected body" "out=$out data_dir=$data_dir"
fi
[ -n "$data_dir" ] && rm -rf "$data_dir"
teardown

# -- Test 20: --compact-directive-file reads file content --------------------

setup
printf 'from-file-content\nline two\n' > "$TDIR/dir.txt"
out="$("${ROOST_BIN}" spawn testnick --cwd "$TDIR" --compact-directive-file "$TDIR/dir.txt" 2>&1 || true)"
if echo "$out" | grep -qF "compact directive: from-file-content"; then
  ok "--compact-directive-file content shown"
else
  fail "--compact-directive-file content shown" "out=$out"
fi
teardown

# -- Test 21: --compact-directive + --compact-directive-file mutually exclusive

setup
printf 'x' > "$TDIR/dir.txt"
err="$("${ROOST_BIN}" spawn testnick --cwd "$TDIR" --compact-directive a --compact-directive-file "$TDIR/dir.txt" 2>&1)"
exit_code=$?
if [ "$exit_code" -ne 0 ] && echo "$err" | grep -q "mutually exclusive"; then
  ok "--compact-directive + --compact-directive-file mutually exclusive"
else
  fail "--compact-directive + --compact-directive-file mutually exclusive" "exit=$exit_code err=$err"
fi
teardown

# -- Test 22: --compact-directive-file missing path errors clearly -----------

setup
err="$("${ROOST_BIN}" spawn testnick --cwd "$TDIR" --compact-directive-file "$TDIR/no-such-file" 2>&1)"
exit_code=$?
if [ "$exit_code" -ne 0 ] && echo "$err" | grep -q "not found"; then
  ok "--compact-directive-file missing path errors"
else
  fail "--compact-directive-file missing path errors" "exit=$exit_code err=$err"
fi
teardown

# -- Test 23: agent's `## Compaction Directive` section extracted ------------

setup
mkdir -p "$TDIR/.claude/agents"
cat > "$TDIR/.claude/agents/withdirective.md" <<'EOF'
---
name: withdirective
model: opus
permissionMode: auto
...
agent-baked-directive-body
keep this too

## Other section
unrelated
EOF
out="$("${ROOST_BIN}" spawn testnick --agent withdirective --cwd "$TDIR" 2>&1 || true)"
if echo "$out" | grep -qF "compact directive: agent-baked-directive-body"; then
  ok "agent's ## Compaction Directive section extracted"
else
  fail "agent's ## Compaction Directive section extracted" "out=$out"
fi
teardown

# -- Test 24: agent without directive section → no diagnostic ----------------

setup
mkdir -p "$TDIR/.claude/agents"
cat > "$TDIR/.claude/agents/bare.md" <<'EOF'
---
name: bare
model: opus
permissionMode: auto
---
Just a body, no Compaction Directive section.
EOF
out="$("${ROOST_BIN}" spawn testnick --agent bare --cwd "$TDIR" 2>&1 || true)"
if ! echo "$out" | grep -q "compact directive:"; then
  ok "no directive in agent → no diagnostic"
else
  fail "no directive in agent → no diagnostic" "out=$out"
fi
teardown

# -- Test 25: --compact-directive overrides agent's section ------------------
...
agent-baked
EOF
out="$("${ROOST_BIN}" spawn testnick --agent withdirective --cwd "$TDIR" --compact-directive 'cli-wins' 2>&1 || true)"
if echo "$out" | grep -qF "compact directive: cli-wins" && ! echo "$out" | grep -qF "agent-baked"; then
  ok "--compact-directive overrides agent section"
else
  fail "--compact-directive overrides agent section" "out=$out"
fi
teardown
