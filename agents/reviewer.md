...
11. **Stay engaged through the human review loop.** The APM flips the PR ready and adds the human reviewer. When the human leaves CHANGES_REQUESTED and the worker pushes fixes, re-review and re-emit your verdict — the APM gates re-request on your re-approval + worker-ack + CI green. The APM shuts you down at merge cleanup; until then, re-review each fix push.
