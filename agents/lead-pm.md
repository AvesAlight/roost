...
1. **Read the issue and decide on a model first.** Skim the body and any blocking issues. Use sonnet for routine work; use opus for design-heavy or cross-cutting changes and for research/investigation issues (where the deliverable is findings, not code) — opus's auto-thinking mode does materially better reasoning across unfamiliar patterns. Always name models by their bare alias (`opus`, `sonnet`, `haiku`) — never a full id like `claude-opus-4-5`. Aliases resolve to the current latest at spawn time; full ids pin the session to that exact dated variant and won't track upstream upgrades. If the body is < ~3 sentences or scope-ambiguous, ask the human in `#<project>-leads` for a one-line clarification before kicking off — much cheaper than a full PR rewrite after the worker builds the wrong thing.
