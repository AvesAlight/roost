...
The APM handles eight dances for you: setup (worktree + watch + worker spawn), reviewer-spawn (when worker posts a draft PR), ready-for-review (mark-ready + add human reviewer + re-request after CHANGES_REQUESTED), merge + cleanup, follow-up filing (`gh issue create` against the current or a named milestone), historian (postmortem comment + learning proposal), prune-learning (remove a stale learning), and milestone teardown (stop dispatcher + APM shutdown). You drive the judgment around each dance — model selection, plan pressure-testing, human review decisions, and whether a follow-up is in scope or pushes the milestone wider; the APM types the commands.
...
9. **Post a postmortem** by mentioning the APM: `<project>-apm postmortem #<I>: <narrative>`. The APM scribes it to a separate comment on the closed issue (the token-cost comment was posted at merge).
   ...
   **Question seeds** (not mandatory, but help surface learnable material):
   ...
   To **prune** a stale learning (the tool it references was removed, the process it describes has changed, or a later learning supersedes it), mention the APM: `<project>-apm prune learning #<I>: <reason>`. The APM acks with the resolved entry header, the files affected, and the reason before mutating — last chance to catch a typo'd issue number. See the prune-learning dance in associate-pm.md.

10. **When the milestone is done** (all issues merged), re-read `.claude/rules/project-learnings.md` and the role learning files; mention the APM to prune any entries the milestone made stale (per step 9's prune vocabulary). Teardown is the natural re-read moment, and an unpruned stale learning compounds across every future session. Then trigger the APM's milestone teardown dance (see associate-pm.md) by mentioning it: `<project>-apm milestone done, stand down`. The APM owns the dispatcher-stop and its own shutdown — wait for `dispatcher stopped, shutting down` in `#<project>-leads`. If no confirmation arrives within ~30s (APM crashed mid-teardown), call `"$(roost root)/bin/stop-dispatcher" "$(pwd)/.orchestrator"` yourself. Then: `roost shutdown <project>-lead-pm`.
