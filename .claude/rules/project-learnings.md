...
`roost spawn --model opus` resolves to the current latest opus at spawn time; `roost spawn --model claude-opus-4-5` pins to that exact dated variant and won't track upstream upgrades. The APM was filling `<model>` placeholders in its templates with full ids and silently shipping work on stale models. Stick to `opus` / `sonnet` / `haiku` unless the lead explicitly asked for a pinned version (rollback test, regression repro, A/B). The wrapper now warns when `--model` looks like a full id — heed it.
